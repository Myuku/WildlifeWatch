...
async function getTraits(animal, location, appearance) {
  let messages = [
    {
      role: "system",
      content:
        "The user will provide a name of an animal. Respond concisely with information on their common name, scientific name, visual appearance, conservation status, species range, diet, how aggressive they are, and diurnality.",
    },
    {
      role: "system",
      content:
        "The user may optionally provide the location of the image taken, by name or coordinates, as well as a description of its appearance. You may use this information to refine your response, such as identifying the subspecies local to the area.",
    },
  ];
  ...
  const response_format = zodResponseFormat(
    AnimalTraitsResponse,
    "animal_traits_response"
  );
...
async function identifyAnimalLabel(labels) {
  ...
  const data = {
    ...
    messages: [
      {
        role: "system",
        content: "You are an AI that identifies animals from labels.",
      },
...
app.get("/image-analysis", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({
        error: "An error occurred while retrieving image analysis records.",
      });
  }
});
...
app.get("/image-analysis/:id", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({
        error: "An error occurred while retrieving the image analysis.",
      });
  }
});
  ...
  try {
    ...
    if (bestLabels.length > 0) {
      ...
      if (animalLabel !== "Error identifying animal label.") {
        ...
        const storedImage = await storeImageDetailsWithBinary(
          imageBinary,
        ...
        return res
          .status(400)
          .json({ error: "Animal label could not be identified." });
      }
    } else {
      return res
        .status(400)
        .json({ error: "No labels detected in the image." });
    }
  } catch (error) {
